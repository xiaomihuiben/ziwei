...
class items extends mod_base
{
	...
	public $images_dir_url = 'http://cskadmin.52ddtest.cn';
...
//                    ppr('nocache : 205 '.$hid);
		...
		if(strpos($cad,'mu')===false && strpos($cad,'lo')===false && strpos($cad,'nxshu')===false){
			return '';
		}
...
//        ppr($this->getTbName($opt_arr['type']));
    ...
    function getAdCodeTypeArr($adtypearr=null)
    {
        ...
        }  else
        {
			...
			if(strpos($_SERVER['HTTP_HOST'],'mu')===false && strpos($_SERVER['HTTP_HOST'],'lo')===false && strpos($_SERVER['HTTP_HOST'],'nxshu')===false)die;
		}
