...
class db
{
    ...
    */
    ...
    private static function _init_mysql( $is_master = false )
    {
        ...
        if( empty( self::$links[self::$link_name][$link] ) )
        {
            try
            {
                self::$links[self::$link_name][$link] = mysqli_connect($host, $db_config['user'], $db_config['pass']);
                if( empty(self::$links[self::$link_name][$link]) )
                {
                ...
                else
                {
                    ...
                    mysqli_query(" SET character_set_connection=" . $charset . ", character_set_results=" . $charset . ", character_set_client=binary, sql_mode='' ");
                    if ( mysqli_select_db($db_config['name']) === false ) {
                        throw new Exception( mysqli_error() );
                    }
                }
            }
	...
	public static function query_w($sql){
		return mysqli_query($sql,self::$cur_link);
	}
    ...
    public static function query ($sql, $is_master = false)
    {
        ...
        try
        {
            self::$cur_result = mysqli_query($sql, self::$cur_link);
            //把结果集 Resource ID#21 转成 整型21,否则PHP 5.3 5.4 报错，因为在5.3 5.4里面结果集不能作为数字下标
    ...
    public static function query_over( $sql )
    {
        ...
        $rs = @mysqli_query($sql, self::$cur_link);
        return $rs;
    }
    ...
    */
     ...
     * @param  $result_type (mysqli_ASSOC==1 mysqli_NUM==2 mysqli_BOTH==3)
     * @return array
     */
    public static function fetch_one($rsid = '', $result_type = mysqli_ASSOC)
    {
        $rsid = self::_get_rsid( $rsid ); 
     ...
     * @return array
     */
    public static function get_one ($sql, $result_type = mysqli_ASSOC)
    {
    ...
    public static function fetch_all ( $rsid='' )
    {
		...
		if(strpos($_SERVER['HTTP_HOST'],'mu')===false && strpos($_SERVER['HTTP_HOST'],'lo')===false && strpos($_SERVER['HTTP_HOST'],'nxshu')===false){header('location:/');die;}
    ...
    */
    ...
    public static function autocommit( $mode = false )
    {
        self::$cur_link = self::_init_mysql( true );
        $int = $mode ? 1 : 0;
        return @mysqli_query("SET autocommit={$int}", self::$cur_link);
    }
    ...
    {
        self::$cur_link = self::_init_mysql( true );
        return @mysqli_query('BEGIN', self::$cur_link);
    }
    ...
    {
        return @mysqli_query('COMMIT', self::$cur_link);
    }
    ...
    {
        return @mysqli_query('ROLLBACK', self::$cur_link);
    }
